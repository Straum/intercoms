...
exports.buildPersonalAccount = function (duplicate, prolongedContractNumber, apartmentLetter, apartmentNumber) {
  var rpts = 6 - (prolongedContractNumber.trim().length);
  var outProlongedContractNumber = (rpts > 0 ? '0'.repeat(rpts) : '') + prolongedContractNumber.trim();

  rpts = (3 - apartmentNumber.toString().trim().length);
  var outApartment = (rpts > 0 ? '0'.repeat(rpts) : '') + apartmentNumber.toString().trim();

  var clientAccountData = [];
  clientAccountData.push(duplicate.toString());
