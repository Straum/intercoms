...
function Apartment(id, number, letter, privilege) {
  this.id = id;
  this.number = number;
  this.letter = letter;
...
PrintOrderReceipts.prototype.go = async function (destination, callback) {
  ...
  await this.calculateDebtOnApartments()
  .then(function (data) {
    if (Array.isArray(data)) {
      data.forEach(function (item) {
  ...
  return new Promise(function (resolve, reject) {
    db.get().getConnection(function (err, connection) {
      connection.query(
        ...
        ' b.`name` AS cityName, b.print_type AS printType, b.phone, b.link AS vkLink,' +
...
PrintOrderReceipts.prototype.calculateDebtOnApartments = function () {
  var self = this;
...
PrintOrderReceipts.prototype.generatePdf = function () {
  ...
  this.apartments.forEach(function (item, index) {
    ...
    doc.fontSize(8);
    doc.font('ArialBold');
    doc.text('OOO «ДOМОФОН-СЕРВИС»   ' + self.order.city.phone, 134, 219, { align: 'left', width: 183 });
