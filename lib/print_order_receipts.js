...
PrintOrderReceipts.prototype.printNicePersonalAccount = function (personalAccount) {
...
PrintOrderReceipts.prototype.generatePdf = function () {
  ...
  this.apartments.forEach(function (item, index) {
    ...
    if (item.exempt === 0) {
      ...
      if (debt != 0) {
        doc
          .text('Задолженность за период с ' + moment(self.order.startService).subtract(1, 'years').format('DD.MM.YYYY') + ' по ' + moment(self.order.endService).subtract(1, 'years').format('DD.MM.YYYY') + ', руб.', 154, 165, { align: 'left', width: 342 })
