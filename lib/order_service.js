...
exports.decodeClientData = function (data) {
  ...
  if (typeof data === 'object') {
    if (('document_name' in data) && (data.document_name !== null)  && (data.document_name.trim().length > 0)) {
      box.passport = data.document_name.trim();
    }
    if (('series' in data) && (data.series.trim().length > 0)) {
      box.passport += ' серия ' + data.series.trim();
    }
    if (('number' in data) && (data.number.trim().length > 0)) {
      box.passport += ' № ' + data.number.trim();
    }
    if (('issue_date' in data) && (data.issue_date !== null)) {
      box.passport += ' ' + moment(data.issue_date).format('DD.MM.YYYY');
    }
    if (('issue' in data) && (data.issue.trim().length > 0)) {
