...
exports.getApartmentsFromContract = function (id, cb) {
  db.get().getConnection(function (err, connection) {
    connection.query(
      ' SELECT' +
      ' a.apartment_id AS uid,' +
      ' a.paid,' +
      ' a.`number`,' +
      ' a.letter,' +
      ' CASE' +
			' WHEN a.letter = 0 THEN a.`number`' +
			' WHEN a.letter = 1 THEN CONCAT(a.`number`, "А")' +
			' WHEN a.letter = 2 THEN CONCAT(a.`number`, "Б")' +
			' WHEN a.letter = 3 THEN CONCAT(a.`number`, "В")' +
			' WHEN a.letter = 4 THEN CONCAT(a.`number`, "Г")' +
			' WHEN a.letter = 5 THEN CONCAT(a.`number`, "Д")' +
			' WHEN a.letter = 6 THEN CONCAT(a.`number`, "Е")' +
			' WHEN a.letter = 7 THEN CONCAT(a.`number`, "М")' +
			' WHEN a.letter = 8 THEN CONCAT(a.`number`, "З")' +
			' WHEN a.letter = 9 THEN CONCAT(a.`number`, "К")' +
		  ' END AS fullNumber,' +
      ' a.privilege,' +
      ' a.exempt,' +
...
exports.decodeClientData = function (data) {
  ...
  if (typeof data === 'object') {
    ...
    //
    if (('registered_city_id' in data) && (data.registered_city_id) && (data.registered_city_id > 0)) {
      if (('registered_city_name' in data) && (data.registered_city_name) && (data.registered_city_name.trim().length > 0)) {
        box.registeredAddress = data.registered_city_name.trim();
      }
      if (('registered_street_name' in data) && (data.registered_street_name) && (data.registered_street_name.trim().length > 0)) {
        box.registeredAddress += ' ' + data.registered_street_name.trim();
      }
      if (('registered_house_number' in data) && (data.registered_house_number) && (data.registered_house_number.trim().length > 0)) {
        box.registeredAddress += ' ' + data.registered_house_number.trim();
      }
      if (('registered_apartment' in data) && (data.registered_apartment) && (data.registered_apartment.trim().length > 0)) {
        box.registeredAddress += ' кв. ' + data.registered_apartment;
      }
    }
    //
    if (('actual_city_id' in data) && (data.actual_city_id) && (data.actual_city_id > 0)) {
      if (('actual_city_name' in data) && (data.actual_city_name) && (data.actual_city_name.trim().length > 0)) {
        box.actualAddress = data.actual_city_name.trim();
      }
      if (('actual_street_name' in data) && (data.actual_street_name) && (data.actual_street_name.trim().length > 0)) {
        box.actualAddress += ' ' + data.actual_street_name.trim();
      }
      if (('actual_house_number' in data) && (data.actual_house_number) && (data.actual_house_number.trim().length > 0)) {
        box.actualAddress += ' ' + data.actual_house_number.trim();
      }
      if (('actual_apartment' in data) && (data.actual_apartment) && (data.actual_apartment.trim().length > 0)) {
