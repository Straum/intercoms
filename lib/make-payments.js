...
MakePayments.prototype.start = async function (destination, callback) {
  ...
  if (this.gates.lines.length > 0) {
    ...
    this.info.lines.push('');
    information = [...this.info.lines, ...this.gates.lines];
    information.push('');
...
MakePayments.prototype.insertData = function (data) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    console.log(`CALL enter_payment(${data.personalAccount},"${data.apartment}","${data.letter}","${data.paymentDateTime}",${data.amount},${data.payMonth},${data.payYear},"${data.fileName}","${data.uniqueCode}","${data.zipCode}",${data.organizationId})`);
