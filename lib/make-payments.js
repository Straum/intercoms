    ...
    switch (words.length) {
      ...
      case 3:
        if ((words[0].trim().length === 1) && ((words[1].trim().length >= russianPost.marker.length) && (words[1].trim().substr(0, 2) == russianPost.marker))) {
          ...
          // FIXME: Duplication
          fs.rename(this.processFile.fullPath, path.join(outPath, '/', fileInfo.base), function (err) {
            if (err) {
              console.log('Ошибка при перемещении файла ' + this.processFile.fullPath + '. Описание ошибки: ' + err);
            }
          });
  ...
  for (const element of lines) {
    ...
    if (element.trim().length != 0) {
      ...
      if ((Array.isArray(segments)) && (segments.length === 3)) {
        ...
        modelRussiaPost.uniqueTransactionCode = rowNumber.toString() + '_' + modelRussiaPost.zipCode + '_' + modelRussiaPost.personalAccount; // emulate unique transaction
