...
function getFullApartment(data) {
  Payer.isDuplicate = Number(modelSberbank.personalAccount.substr(0, 1)) > 0;
  Payer.prolongContractNumber = modelSberbank.personalAccount.substr(1, 6);
  Payer.letter = modelSberbank.personalAccount.substr(7, 1);
  Payer.apartment = modelSberbank.personalAccount.substr(8, 3);
  ...
  for (const element of lines) {
    ...
    if ((element.trim().length != 0) && (element.substr(0, 1) != '=')) {
      ...
      if ((Array.isArray(segments)) && (segments.length <= 13)) {
        ...
        prepareData.apartment = info.apartment;
